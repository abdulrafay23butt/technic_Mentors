...
function Layout() {
    ...
    async function getNotification() {
        try {
            const response = await fetch(`http://localhost:3000/getNotification?person=${user}`);
    ...
    return login ? (
        <>
            <div id="mySidebar" className="sidebar">
                <img src={logo} alt="" />
                <Link to="/Layout" onClick={() => { setpage("DashBoard") }}><p className='d-flex mt-3'><FontAwesomeIcon className='fs-5 me-1 ' icon={faGaugeHigh} />DashBoard</p></Link><hr />
                ...
                <div id="projectManagement" className='collapse'>
                    <Link to="project" ><p className='d-flex ms-3'><FontAwesomeIcon className='fs-5 me-1 ' icon={faBarsProgress} />Projects</p></Link><hr />
                    <a href="#"><p className='d-flex ms-3'><FontAwesomeIcon className='fs-5 me-1 ' icon={faBarsProgress} />Project Catogorizes</p></a><hr />
                    <Link to="" ><p className='d-flex ms-3'><FontAwesomeIcon className='fs-5 me-1 ' icon={faListCheck} />Assign Projects</p></Link><hr />
                </div>
                ...
                <div id="employeeManagment" className='collapse'>
                    ...
                    <Link to="leave" onClick={() => { setpage("Leave Requests") }}><p className='d-flex ms-3'><FontAwesomeIcon className='fs-5 me-1 ' icon={faUserXmark} />Leave Request</p></Link><hr />
                </div>
                ...
                <div className="collapse" id='taskManagment'>
                    <Link to="todo"><p className='d-flex ms-3'><FontAwesomeIcon className='fs-5 me-1 ' icon={faTableCellsLarge} />Todo's</p></Link><hr />
                                    ...
                                    <ul className="list-unstyled">
                                        {notifications.length > 0 ? (notifications.slice(0, 5).map((notification) => (
                                            ...
                                            <li key={notification._id} className="d-flex  mb-2 list-group-item notification_button">
                                                ...
                                                <div>
                                                    <button
                                                        className='btn m-0 p-0 border-0 d-flex justify-content-start align-items-start'
                                                        onClick={() => {
                                                            if (notification.Reason === "Leave") { navigate("leave") }
                                                            else if (notification.Reason === "Project") { navigate("project")}
                                                        }}>
                                                        {notification.Reason === "Leave" ? (<p className=" m-0 mb-1">The status of your leave request has been changed</p>) : (notification.Reason === "Project" ? (<p className=" m-0 mb-1">New Project Has Been Assigned</p>) : null)}
                                                        <small className="text-muted  ms-4">{notification.Date}</small>
                                                    </button>
                                                </div>
                                                <hr />
                                            </li>
